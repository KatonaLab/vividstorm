...
class Settings(object):
    ...
    def _setup_components(self, main_window, type_):
        widgets = main_window.__dict__.keys()
...
class StormSettings(Settings):
    ...
    def apply_storm_filters(self):
        image = self.main_window.storm_images_list.currentItem()
        if image:
            self.main_window.viewer.show_storm_image(image)
        else:
...
class ConfocalSettings(Settings):
    ...
    def set_confocal_config(self):
        ...
        for radio_button in self.radio_buttons:
            ...
            if radio_button.isChecked() and obj_name.find('confocal_config_resize') != -1:
                resampling_method = obj_name.split('_')[-1]
        self.main_window.viewer.display.SetConfocalInterpolationMethod(resampling_method)
    ...
    def apply_confocal_config(self):
        ...
        if image:
            image.isParsingNeeded = True
            self.main_window.viewer.show_confocal_image(image,ApplyButton=True)
        else:
